...
const columns = [
    ...
    },
    {
        ...
        sortable: false,
        // renderCell: (params) => (
        //     <TableTotalValue data={[params, params.row.totalConsignee]} />
        // )
    },
    {
        field: 'noOfWagons',
    ...
    render(){
        return(
            <Stack direction={'column'} alignItems={'end'} width={1100}>
                ...
                <Box width={1100}>
                        <Typography variant="h5" fontWeight={800}>Datewise List of Rakes Weighed Grouped by Consigneee From {this.fromDate} Till {this.tillDate}</Typography>
                </Box>
                <Box width={1}>
                    <DataGrid
                        initialState={{
